...
- Avalonia tests：95 / 95 passed；28 張 approved screenshots 由 exact pixel gate 驗證。
...
目前驗證基線：core/parser/rendering/CLI 289 tests passed；Avalonia 95 tests passed；28 張 approved PNG exact compare 通過。私有 Common KingstVIS／DSL／NDS smoke 已於 `022ba60` 後重跑。尚未關閉的 release gate 為私有 Desay golden、125% UI scale/相關 visual baseline、正式 tag 與 package/release publish。
...
- [x] Core / parser / rendering / CLI 289/289；Avalonia 95/95。
