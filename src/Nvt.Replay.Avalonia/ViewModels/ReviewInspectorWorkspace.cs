...
public sealed class ReviewInspectorWorkspace
{
        ...
        TimeProvider? timeProvider = null)
    {
        ...
        if (markers is not null)
        {
            foreach (var marker in markers)
            {
                ...
                if (this.markers.Any(item => item.Id.Equals(marker.Id, StringComparison.Ordinal)))
                    throw new ArgumentException($"Duplicate marker ID '{marker.Id}'.", nameof(markers));
    ...
    private void RebuildReviewSession(string? preferredGroupId = null)
    {
        ...
        RefreshVisibleGroups();
        if (priorGroupId is null) return;
        ...
        return new ReplayDiagnostic(
            ...
            new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                ["evidence"] = evidence.Count == 0
                    ? "-"
                    : string.Join("; ", evidence.Select(item => $"{item.Kind}:{item.Label ?? Path.GetFileName(item.Path)}")),
            });
    }
