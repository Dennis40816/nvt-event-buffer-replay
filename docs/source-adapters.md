...
| `nds-communication-log` | NDS Paint/Read/Write text records, including continuation lines. The address token is overloaded by both operation and range: `Paint TP 0x00..0x7F` means an Event Buffer read from that 7-bit I2C slave and does not contain a register byte; `Paint TP >=0x100` is an absolute-register read; every Read/Write token is an absolute register even when it is small (for example `Write TP 0x62`). The ambiguous `0x80..0xFF` Paint range is warned and discarded. Known registers are annotated through the shared register catalog and FW Command parser; payload bytes remain immutable. |
